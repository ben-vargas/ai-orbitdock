...
impl WatcherRuntime {
  ...
  pub(crate) async fn handle_rollout_events(
    ...
    for event in events {
      match event {
        RolloutEvent::SessionMeta {
          ...
          self
            .handle_session_meta_event(SessionMetaEvent {
              session_id,
              cwd,
              model_provider,
              originator,
        ...
        RolloutEvent::TurnContext {
          ...
          self
            .handle_turn_context_event(session_id, project_path, model, effort)
            .await;
        }
        ...
        RolloutEvent::AppendChatMessage {
          ...
          self
            .append_chat_message(AppendChatMessageArgs { session_id, row })
            .await;
        }
        RolloutEvent::ShellCommandBegin {
          ...
          self
            .append_rollout_shell_message(&session_id, &call_id, command)
            .await;
        }
        RolloutEvent::ShellCommandEnd {
          ...
          self
            .finish_rollout_shell_message(&session_id, &call_id, output, is_error, duration_ms)
            .await;
        }
    ...
    for (session_id, pending) in coalesced.work_state {
      self
        .update_work_state(WorkStateUpdate {
          session_id,
    ...
    for (session_id, pending) in coalesced.token_counts {
      self
        .handle_token_count(&session_id, pending.total_tokens, pending.token_usage)
        .await;
    }
    ...
    for row in rows {
      actor
        .send(SessionCommand::ProcessEvent {
          event: crate::domain::sessions::transition::Input::RowCreated(ConversationRowEntry {
  ...
  async fn update_work_state(&mut self, update: WorkStateUpdate) {
    ...
    let has_attention_payload =
      pending_tool_name.is_some() || pending_tool_input.is_some() || pending_question.is_some();
    let _ = self
      .persist_tx
      .send(PersistCommand::RolloutSessionUpdate {
        id: session_id.to_string(),
...
"#,
  ...
  async fn process_file_persists_checkpoint_snapshot() {
    ...
    let tmp_dir = std::env::temp_dir().join(format!("orbitdock-rollout-checkpoint-{}", session_id));
    std::fs::create_dir_all(&tmp_dir).expect("create temp dir");
    let rollout_path = tmp_dir.join("rollout.jsonl");
    std::fs::write(
      &rollout_path,
    ...
    let (path, offset, persisted_session_id) = saw_checkpoint.expect("checkpoint persist command");

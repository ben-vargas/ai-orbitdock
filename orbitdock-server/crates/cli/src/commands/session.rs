...
use orbitdock_protocol::{
    ...
    SessionListItem, SessionState, SessionStatus, SessionSummary, SessionSurface,
        ...
        Ok(snapshot) => {
            if output.json {
                ...
                let conversation = if messages {
                    if let Some(snapshot_page) =
                        conversation_snapshot_from_session(&snapshot.session)
                    {
                        Some(snapshot_page)
...
mod tests {
    ...
    fn session_json_overview_omits_impossible_cache_hit_percent() {
        ...
        let value = serde_json::to_value(session_json_overview_from_state(&state))

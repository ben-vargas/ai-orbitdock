...
async fn fetch_codex_usage(rest: &RestClient) -> Result<CodexUsageResponse, (i32, CliError)> {
    rest.get::<CodexUsageResponse>("/api/usage/codex")
        .await
        .into_result()
}

async fn fetch_claude_usage(rest: &RestClient) -> Result<ClaudeUsageResponse, (i32, CliError)> {
    rest.get::<ClaudeUsageResponse>("/api/usage/claude")
        .await
        .into_result()
}
...
mod tests {
    ...
    use super::{
        build_usage_json_response, ClaudeUsageResponse, CodexUsageResponse, ProviderFilter,
    };

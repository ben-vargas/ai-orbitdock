...
struct APIClientTests {
  ...
  @Test func convertsWebSocketURLsIntoHTTPBaseURLs() throws {
    ...
    #expect(APIClient.httpBaseURL(from: secure).absoluteString == "https://example.com")
    #expect(APIClient.httpBaseURL(from: insecure).absoluteString == "http://127.0.0.1:4000")
    #expect(APIClient.httpBaseURL(from: nested).absoluteString == "https://example.com/orbitdock")
  }
  ...
  @Test func setServerRoleSendsScopedJSONAndAuthorizationHeader() async throws {
    ...
    let isPrimary = try await client.setServerRole(isPrimary: true)
  ...
  @Test func browseDirectoryPreservesPathQueryAndDecodesListing() async throws {
    ...
    let result = try await client.browseDirectory(path: "/tmp/project")
    let request = try #require(await recorder.singleRequest())
  ...
  @Test func surfacesStructuredHTTPFailures() async throws {
    ...
    do {
      _ = try await client.fetchSessionSnapshot("missing-session")
